...
export const conversations = pgTable("conversations", {
  ...
  // Email threading metadata for conversation-level tracking
  threadMessageIds: text("thread_message_ids").array(), // Array of all Message-IDs in this thread
  originalMessageId: varchar("original_message_id", { length: 255 }), // First Message-ID that started this thread
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
});
...
export const conversationMessages = pgTable("conversation_messages", {
  ...
  // Email threading support for proper client threading
  messageId: varchar("message_id", { length: 255 }), // RFC 5322 Message-ID
  inReplyTo: varchar("in_reply_to", { length: 255 }), // Message-ID this is replying to
  references: text("references"), // Space-separated list of Message-IDs in thread
  emailHeaders: jsonb("email_headers").default(sql`'{}'::jsonb`).notNull(), // Raw email headers for debugging
  createdAt: timestamp("created_at").defaultNow().notNull(),
});
