...
import { chatCampaign, APIError } from '@/api/client';
...
export default function AIChatInterface() {
  ...
  const chatMutation = useMutation({
    mutationFn: async (userMessage: string) =>
      chatCampaign({ message: userMessage, currentStep, campaignData }),
    ...
    onError: (err, variables) => {
      ...
      let content = "I'm having trouble reaching the server. Rephrase or try again in a moment.";
      if (err instanceof APIError) {
        const errorMessages: Record<string, string> = {
          network: "I couldn't reach the server. Please check your connection and try again.",
          api: "The AI service is unavailable right now. Please retry shortly.",
          auth: "Authentication error. Please refresh the page and try again.",
          validation: "Something about that input doesn't look right. Could you rephrase?",
          rate_limit: "I'm getting too many requests. Please wait a moment and try again.",
          unknown: "I'm having trouble reaching the server. Rephrase or try again in a moment."
        };
        content = errorMessages[err.type] || errorMessages.unknown;
      }
