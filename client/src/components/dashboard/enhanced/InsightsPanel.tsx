  ...
  const realMetrics = [
    {
      ...
      change: Math.round((campaigns.filter(c => c.status === 'active').length / Math.max(campaigns.length, 1)) * 100 - 50),
      icon: MessageSquare,
    ...
    },
    {
      ...
      change: campaigns.length > 5 ? 15 : campaigns.length > 2 ? 8 : -5,
      icon: Users,
    ...
    },
    {
      ...
      change: Math.round((campaigns.filter(c => c.status === 'completed').length / Math.max(campaigns.length, 1)) * 100 - 30),
      icon: Target,
