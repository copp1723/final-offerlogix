...
import { useChatCampaign } from '@/hooks/useDashboardIntelligence';
...
export function AIAgentPanel({ suggestions, recentActivity }: AIAgentPanelProps) {
  ...
  const [chatHistory, setChatHistory] = useState<Array<{ role: 'ai' | 'user'; message: string }>>([
    {
      role: 'ai',
      message: "Hello! I'm your AI Campaign Agent. Tell me your campaign idea and I'll help you build it using our Memory over Metrics approach - focusing on what helps you close deals, not vanity numbers.",
    },
  ]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const { chatCampaign } = useChatCampaign();
  ...
  const handleSend = async () => {
    ...
    try {
      setLoading(true);
      const res = await chatCampaign(userMsg);
      setChatHistory((h) => [...h, { role: 'ai', message: res.message || 'Let me help you build that campaign with actionable insights, not just metrics.' }]);
    } catch (e) {
      if (e instanceof APIError) {
        const errorMessages: Record<string, string> = {
          network: 'Network error. Please check your connection and try again.',
          api: 'AI service is currently unavailable. Please retry shortly.',
          auth: 'Authentication error. Please refresh the page.',
          validation: 'Please check your input and try again.',
          rate_limit: 'Too many requests. Please wait a moment.',
          unknown: 'Failed to reach AI service'
        };
        const message = errorMessages[e.type] || errorMessages.unknown;
        setError(message);
        setChatHistory((h) => [...h, { role: 'ai', message }]);
      } else {
        setError('An unexpected error occurred');
        setChatHistory((h) => [...h, { role: 'ai', message: "I'm having trouble connecting right now, but I'm here to help you create campaigns that focus on real context - like 'Bob needs 7-seater for twins' rather than 'Lead Score: 85/100'." }]);
      }
    } finally {
      setLoading(false);
    }
    ...
    <div className="space-y-6">
      ...
      <div className="relative overflow-hidden glass rounded-3xl p-12 shadow-glow">
        ...
        <div className="relative text-center">
          ...
          <p className="text-gray-600 text-lg">Memory over Metrics - Create campaigns that focus on actionable context</p>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-white/80 text-sm">Memory over Metrics AI</p>
              </div>
            </div>
          ...
          {suggestions.map((suggestion, idx) => (
            ...
            >
              ...
              <div className="mt-3 flex items-center gap-2">
                <span className="text-xs text-purple-600 font-semibold">Context-First</span>
      ...
      <div className="glass rounded-2xl p-6 shadow-glow">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h3 className="font-bold text-xl text-gray-900">Recent Insights</h3>
          <span className="text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded-full font-semibold">Live Data</span>
        </div>
          ...
          {recentActivity.map((activity, idx) => (
            <div key={idx} className="flex items-start gap-4 group">
              ...
              <div className="flex-1 pb-2">
                ...
                <p className="text-xs text-gray-500 mt-1">Real insight, not just metrics</p>
              </div>
            </div>
          ))}
        </div>
