...
import { Sparkles, Users, TrendingUp, AlertCircle, Mail, Zap, BarChart3, Activity, MessageSquare, Send } from 'lucide-react';
...
export default function Dashboard() {
  ...
  // Fetch additional data for metrics
  const { data: campaigns = [] } = useQuery({
    queryKey: ["/api/campaigns"],
    queryFn: () => queryClient.fetchQuery({ queryKey: ["/api/campaigns"] })
  });

  const { data: conversations = [] } = useQuery({
    queryKey: ["/api/conversations"],
    queryFn: () => queryClient.fetchQuery({ queryKey: ["/api/conversations"] })
  });
  ...
  const tabs = [
    ...
    { value: 'leads' as const, label: 'Dealership Leads', icon: Users, count: data.leads.length, color: 'from-blue-500 to-cyan-500' },
          ...
          <div className="py-6">
                ...
                {data.summary.hotLeadsNeedingAttention > 0 && (
                  <div className="glass px-4 py-2 rounded-xl border border-red-200/50 shadow-lg">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-xs text-gray-600">Priority Leads</p>
            ...
            <div className="grid grid-cols-2 md:grid-cols-6 gap-4 mt-6">
              ...
              <div className="glass rounded-xl p-4 hover:shadow-glow transition-all duration-300 transform hover:scale-105">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs text-gray-600 uppercase tracking-wider">Priority Leads</p>
              ...
              <div className="glass rounded-xl p-4 hover:shadow-glow transition-all duration-300 transform hover:scale-105">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs text-gray-600 uppercase tracking-wider">Campaigns</p>
                    <p className="text-2xl font-bold text-gradient">{campaigns.length}</p>
                  </div>
              ...
              <div className="glass rounded-xl p-4 hover:shadow-glow transition-all duration-300 transform hover:scale-105">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs text-gray-600 uppercase tracking-wider">Active Campaigns</p>
                    <p className="text-2xl font-bold text-gradient">
              ...
              <div className="glass rounded-xl p-4 hover:shadow-glow transition-all duration-300 transform hover:scale-105">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs text-gray-600 uppercase tracking-wider">Conversations</p>
                    <p className="text-2xl font-bold text-gradient">{conversations.length}</p>
                  </div>
              ...
              <div className="glass rounded-xl p-4 hover:shadow-glow transition-all duration-300 transform hover:scale-105">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs text-gray-600 uppercase tracking-wider">Active Convs</p>
                    <p className="text-2xl font-bold text-gradient">
