...
export class CSVValidationService {
  ...
  static async validateCSV(
    ...
    try {
      ...
      // Normalize field names first to handle case variations (e.g., Email -> email)
      records = records.map(record => this.normalizeFieldNames(record));

      // Validate column headers after normalization
      ...
      for (let i = 0; i < records.length; i++) {
        ...
        // Skip completely empty rows (all values are empty strings or null/undefined)
  ...
  private static validateColumns(
    ...
    if (config.requireColumns) {
      ...
      const missingColumns = config.requireColumns.filter(
        col => !headersLowerCase.includes(col.toLowerCase())
      );
  ...
  /**
   * Normalize field names to handle common case variations
   */
  private static normalizeFieldNames(record: any): any {
