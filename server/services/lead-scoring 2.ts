...
// Supermemory integration disabled for now - module doesn't exist
...
export class LeadScoringService {
  ...
  private getAllMessages(conversations: any[]) {
    return conversations.flatMap(c => c.messages || []);
  }

  private getLeadMessages(conversations: any[]) {
    return this.getAllMessages(conversations).filter((m: any) => !m.isFromAI);
  }

  private getAgentMessages(conversations: any[]) {
    return this.getAllMessages(conversations).filter((m: any) => m.isFromAI);
  }

  private toLowerBlob(conversations: any[]) {
  ...
  private evaluateResponseSpeed(conversations: any[]): number {
