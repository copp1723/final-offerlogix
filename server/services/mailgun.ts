...
export async function sendCampaignEmail(
  ...
  options: { isAutoResponse?: boolean; domainOverride?: string; senderName?: string; threadingHeaders?: Record<string, string>; userVariables?: Record<string, string> } = {}
): Promise<boolean> {
  ...
  try {
    ...
    // Enforce allowed domains (no silent fallback)
    const allowed = (process.env.MAILGUN_ALLOWED_DOMAINS || '')
      .split(',')
      .map(d => d.trim())
      .filter(Boolean);
    const defaultDomain = process.env.MAILGUN_DOMAIN || process.env.MAILGUN_DOMAIN_DEFAULT || '';
    if (defaultDomain) allowed.push(defaultDomain);
    if (allowed.length > 0 && domain && !allowed.includes(domain)) {
      console.error(`Blocked send: domain ${domain} not in MAILGUN_ALLOWED_DOMAINS`);
      return false;
    }
    if (!apiKey || !domain) {
    ...
    const senderName = options.senderName || process.env.DEFAULT_SENDER_NAME || 'OneKeel Swarm';

    // Determine the local-part for the sender mailbox.
    // Priority: explicit variables.from override > env MAILGUN_FROM_LOCAL_PART > default 'riley'
    const fromOverride = (typeof variables?.from === 'string' && /<[^>]+>/.test(variables.from)) ? String(variables.from) : undefined;
    const localPart = process.env.MAILGUN_FROM_LOCAL_PART?.trim() || 'campaigns';
    const fromEmail = fromOverride || `${senderName} <${localPart}@${domain}>`;
    ...
    // Sanitize content to remove any bracketed placeholders, then size-cap
    const html = capHtmlSize(sanitizeAutomotiveReply(content || ''));
    ...
    // Ensure replies route back to the same mailbox as From
    // Use the full From header for Reply-To to preserve agent display name
    const replyToFull = fromOverride || `${senderName} <${localPart}@${domain}>`;
    body.append('h:Reply-To', replyToFull);

    // Add threading headers if provided (for email replies)
    if (options.threadingHeaders) {
      console.log('🧵 Adding threading headers to email:', options.threadingHeaders);
      for (const [key, value] of Object.entries(options.threadingHeaders)) {
        if (value) {
          console.log(`🧵 Adding header h:${key} = ${value}`);
          body.append(`h:${key}`, value);
        }
      }
    } else {
      console.log('🧵 No threading headers provided for email');
    }

    // Add Mailgun user-variables for webhook correlation if provided
    if (options.userVariables) {
      for (const [k, v] of Object.entries(options.userVariables)) {
        if (v != null) body.append(`v:${k}`, String(v));
      }
    }
