...
export class AgentRuntime {
  ...
  model: 'openai/gpt-5-chat' as ModelId
    ...
    // Final safety: sanitize reply to remove any bracketed location placeholders
    const safeReply = sanitizeAutomotiveReply(reply.trim() || DEFAULT_FALLBACK_REPLY);
  ...
  static async ensureDefaultConfig(clientId: string): Promise<string> {
    try {
      ...
      const defaultConfig = await db.insert(aiAgentConfigTable).values({
        ...
        isActive: true,
        fromName: 'Swarm Automotive Agent',
