...
export interface IStorage {
  ...
  updateConversationStatus(id: string, status: string): Promise<Conversation | null>;
  ...
  getConversationMessageByMessageId(messageId: string): Promise<ConversationMessage | null>;
  getConversationByOriginalMessageId(messageId: string): Promise<Conversation | null>;
  updateConversationThreading(id: string, messageIds: string[], originalMessageId?: string): Promise<Conversation>;
...
export class DatabaseStorage implements IStorage {
  ...
  async createConversationMessage(message: InsertConversationMessage): Promise<ConversationMessage> {
    ...
    // Update conversation threading if this message has a Message-ID
    if (newMessage.messageId && newMessage.conversationId) {
      try {
        const conversation = await this.getConversation(newMessage.conversationId);
        if (conversation) {
          const currentIds = conversation.threadMessageIds || [];
          const updatedIds = [...currentIds, newMessage.messageId];
          
          // Set original message ID if this is the first message in the conversation
          const originalId = conversation.originalMessageId || 
                           (currentIds.length === 0 ? newMessage.messageId : conversation.originalMessageId);
          
          await this.updateConversationThreading(
        ...
        console.warn('Failed to update conversation threading:', error);
      }
    }
  ...
  // Threading support methods
  async getConversationMessageByMessageId(messageId: string): Promise<ConversationMessage | null> {
    const [message] = await db
      .select()
      .from(conversationMessages)
      .where(eq(conversationMessages.messageId, messageId));
    return message || null;
  }

  async getConversationByOriginalMessageId(messageId: string): Promise<Conversation | null> {
    const [conversation] = await db
      .select()
      .from(conversations)
      .where(eq(conversations.originalMessageId, messageId));
    return conversation || null;
  }

  async updateConversationThreading(id: string, messageIds: string[], originalMessageId?: string): Promise<Conversation> {
    const updateData: any = {
      threadMessageIds: messageIds,
      updatedAt: new Date(),
    };
