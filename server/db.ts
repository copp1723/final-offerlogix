// NOTE:
// - dotenv is loaded once in the main entry (server/index.ts). However, due to import
//   dependency chains (routes.ts -> storage.ts -> db.ts), we need to ensure dotenv
//   is loaded here as well to prevent DATABASE_URL missing errors.
...
async function applyLegacyPatches() {
  ...
  await addColumn('handover_prompt_spec', `ALTER TABLE campaigns ADD COLUMN handover_prompt_spec jsonb`);
    ...
    await addColumn('send_window', `ALTER TABLE campaigns ADD COLUMN send_window jsonb`);
  ...
  await addColumnTo('ai_agent_config', 'model', `ALTER TABLE ai_agent_config ADD COLUMN model text DEFAULT 'openai/gpt-5-chat'`);
    ...
    try {
      ...
      log.warn('Database model default update warning', {
        component: 'database',
        operation: 'model_default_update',
        error: e as Error
      });
    }
    ...
    log.warn('Warning while applying database legacy patches', {
      component: 'database',
