...
router.get('/', async (req, res) => {
  try {
    ...
    const errorContext = createErrorContext(error, { operation: 'get_personas', clientId: req.headers['x-client-id'] as string });
...
router.post('/create-defaults', async (req, res) => {
  try {
    ...
    const errorContext = createErrorContext(error, { operation: 'create_default_personas', clientId: req.headers['x-client-id'] as string });
...
router.post('/', async (req, res) => {
  try {
    ...
    const {
      ...
      priority = 0,
      metadata = {}
      // emailSubdomain field removed as it doesn't exist in the schema
    } = req.body;
    ...
    const errorContext = createErrorContext(error, { 
      ...
      personaData: { name: req.body.name, targetAudience: req.body.targetAudience, industry: req.body.industry }
    });
