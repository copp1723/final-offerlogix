...
var clients, users, conversations, conversationMessages, campaigns, aiAgentConfig, leads, insertUserSchema, insertCampaignSchema, insertAiAgentConfigSchema, insertConversationSchema, insertConversationMessageSchema, insertLeadSchema, insertClientSchema;
var init_schema = __esm({
  "shared/schema.ts"() {
    ...
    aiAgentConfig = pgTable("ai_agent_config", {
      ...
      model: text("model").default("openai/gpt-5-mini"),
  ...
  "server/storage.ts"() {
    ...
    DatabaseStorage = class {
      ...
      async createCampaign(campaign) {
        ...
        try {
          const { MemoryMapper: MemoryMapper2 } = await import("../integrations/supermemory");
...
Goals: ${newCampaign.handoverGoals}`,
      ...
      async getConversationMessages(conversationId, limit) {
        const query = db.select().from(conversationMessages).where(eq(conversationMessages.conversationId, conversationId)).orderBy(desc(conversationMessages.createdAt));
        return limit ? (await query).slice(0, limit) : await query;
      }
      async createConversationMessage(message) {
        const [newMessage] = await db.insert(conversationMessages).values({
          ...message,
          createdAt: /* @__PURE__ */ new Date()
        }).returning();
        if (!message.isFromAI && newMessage.content && typeof newMessage.content === "string") {
          try {
            ...
            await MemoryMapper2.writeLeadMessage({
              ...
              leadEmail: message.senderId || void 0,
...
`;
  try {
    ...
    return result.templates?.map((t) => t.content) || [];
  } catch (error) {
...
async function requestWithRetries(url, init) {
  let attempt = 0;
  while (true) {
    attempt++;
...
async function sendCampaignEmail(to, subject, content, variables = {}, options = {}) {
  try {
    ...
    const fromEmail = options.isAutoResponse ? `OneKeel Swarm <noreply@${domain}>` : options?.isAutoResponse === false && typeof variables?.from === "string" ? variables.from : `OneKeel Swarm <campaigns@${domain}>`;
...
var MAILGUN_TIMEOUT_MS, MAILGUN_MAX_RETRIES, MAILGUN_RETRY_BASE_MS, MAILGUN_MAX_HTML_BYTES, MAILGUN_BULK_CONCURRENCY;
...
async function requestWithRetries2(path4, method, body) {
  ...
  "server/integrations/supermemory/client.ts"() {
    ...
    realClient = API_KEY ? {
      add: async (data) => {
        ...
        const res = await requestWithRetries2("/v1/memories", "POST", payload);
        if (!res.ok) {
      ...
      search: async (params) => {
        ...
        const res = await requestWithRetries2("/v1/search", "POST", body);
        if (!res.ok) {
          const text2 = await res.text().catch(() => "");
...
var init_llm_client = __esm({
  "server/services/llm-client.ts"() {
    ...
    LLMClient = class {
       ...
       */
      static async generateAutomotiveContent(prompt) {
      ...
      async processEmailSequence(campaign, leads2, templateIndex = 0, options = {}) {
        ...
        try {
          ...
          for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
            ...
            const batchResults = batchResultsSettled;
            for (let i = 0; i < batchResults.length; i++) {
              ...
              if (result.status === "fulfilled" && result.value.success) {
                ...
                emailsFailed++;
                const errorMessage = result.status === "rejected" ? result.reason : result.value.error || "Unknown error";
      ...
      findMatchingCampaign(lead, campaigns2, rule) {
        ...
        if (lead.timeframe) {
          const timeframeCampaign = activeCampaigns.find(
            (c) => c.context?.toLowerCase().includes(lead.timeframe.toLowerCase()) || c.name.toLowerCase().includes(lead.timeframe.toLowerCase())
          );
      ...
      async executeCampaign(options) {
        ...
        try {
          ...
          return {
            ...
            message: testMode ? `Test email sent to ${processingResult.emailsSent} lead(s)` : `Campaign executed successfully`,
            emailsSent: processingResult.emailsSent,
            emailsFailed: processingResult.emailsFailed,
  ...
  "server/services/handover-email.ts"() {
    ...
    HandoverEmailService = class {
      ...
      static generateHandoverEmail(data) {
        ...
        const urgencyColor = evaluation.urgencyLevel === "high" ? "#dc2626" : evaluation.urgencyLevel === "medium" ? "#ea580c" : "#16a34a";
        const urgencyIcon = evaluation.urgencyLevel === "high" ? "\u{1F525}" : evaluation.urgencyLevel === "medium" ? "\u26A0\uFE0F" : "\u{1F4CB}";
        return `
<!DOCTYPE html>
<html>
<head>
    ...
    <div class="container">
        ...
        <div class="content">
            ...
            <div class="criteria-list">
                <h4 style="margin-top: 0; color: #92400e;">\u{1F3AF} Handover Reason</h4>
                <p style="margin: 0; font-weight: 600;">${evaluation.reason}</p>
                <h4 style="color: #92400e;">Triggered Criteria:</h4>
                <ul>
                    ${evaluation.triggeredCriteria.map((criteria) => `<li>${criteria.replace("_", " ").toUpperCase()}</li>`).join("")}
                </ul>
            </div>
            ...
            <div class="actions-list">
                ...
                <ul>
                    ${evaluation.nextActions.map((action) => `<li>${action}</li>`).join("")}
                </ul>
            </div>
...
NEXT ACTIONS:
${evaluation.nextActions.map((action) => `- ${action}`).join("\n")}
    ...
    HandoverService = class {
      ...
      static async processHandover(conversationId, evaluation, criteria, additionalData) {
        try {
          ...
          const emailSent = await HandoverEmailService2.sendHandoverNotification({
            conversationId,
            evaluation,
...
`;
    CampaignPromptService = class _CampaignPromptService {
      ...
      static generateContextualPrompt(userInput, campaignType, urgency) {
...
${ragContext}` : llmUserPrompt,
      ...
      static broadcastProgress(campaignId, stepIndex, total, percent) {
        try {
          const broadcast = (init_websocket(), __toCommonJS(websocket_exports))?.broadcastMessage;
          if (broadcast) {
...
${snippets.map((s) => `${s.title ? `${s.title}: ` : ""}${s.content}`).join("\n---\n")}
  ...
  "server/services/agent-runtime.ts"() {
    ...
    AgentRuntime = class _AgentRuntime {
      ...
      static async getActiveConfig(clientId) {
        try {
          const rows = await db.select().from(aiAgentConfig).where(and3(eq7(aiAgentConfig.clientId, clientId), eq7(aiAgentConfig.isActive, true))).limit(1);
...
- ${cfg.dontsList.join("\n- ")}` : "";
      ...
      static async recallMemory(opts) {
        try {
          ...
          let lead = null;
          if (leadId) {
            const rows = await db.select().from(leads).where(eq7(leads.id, leadId)).limit(1);
            lead = rows?.[0] || null;
          }
          const tags = [
            `client:${clientId}`,
      ...
      try {
        ...
        if (currentConfig) {
          await db.update(aiAgentConfig).set({
            ...
            updatedAt: /* @__PURE__ */ new Date()
          }).where(eq8(aiAgentConfig.id, currentConfig.id));
...
var init_enhanced_conversation_ai = __esm({
  "server/services/enhanced-conversation-ai.ts"() {
    ...
    `;
       ...
       */
      async generateAutomotiveResponse(context, newMessage, messageAnalysis, memoryContext, options) {
        const client = getOpenAIClient();
    ...
    Respond as a knowledgeable automotive professional who understands this customer's needs and can guide them toward their perfect vehicle match.`;
       ...
       */
      buildContextualizedPrompt(context, newMessage, messageAnalysis, memoryContext, options) {
