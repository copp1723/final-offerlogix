...
`;
var CampaignPromptService = class _CampaignPromptService {
...
Guidance: Ensure per\u2011segment coverage (subject lines & CTAs). Recommend 6\u20139 templates if more than one segment is present.`;
  ...
  static detectSegmentsFromText(text) {
    if (!text) return [];
    const out = [];
    const boldRe = /\*\*(.+?)\*\*/g;
    let m;
...
export {
  ...
  ENHANCED_AUTOMOTIVE_EMAIL_MARKETING_PROMPT
};
